...
export class StylePaginatorDirective implements AfterViewInit {
  private _pageGapTxt = "...";
  private _rangeStart: number = 1;
  private _rangeEnd: number=5;
        ...
        for (let i = 0; i < nodeArray.length; i++) {
          if (nodeArray[i].nodeName === "BUTTON") {
            if (nodeArray[i].innerHTML.length > 100 && nodeArray[i].disabled) {
              this.ren.addClass(nodeArray[i],'hidden')
            } else if (
              nodeArray[i].innerHTML.length > 100 &&
              !nodeArray[i].disabled
            ) {
              this.ren.addClass(nodeArray[i],'hidden')
              // this.ren.setStyle(nodeArray[i], "background-color", "#009CB2");
  ...
  private createButton(i: any, pageIndex: number): any {
    ...
    this.ren.addClass(linkBtn, "m-1");
    this.ren.addClass(linkBtn, "w-[30px]");
    this.ren.addClass(linkBtn, "h-[30px]");
    this.ren.addClass(linkBtn, "text-[#009CB2]");
    this.ren.addClass(linkBtn, "bg-[#F5F5F5]");
    this.ren.addClass(linkBtn, "rounded-md");
    this.ren.addClass(linkBtn, "font-sans");
    ...
    switch (i) {
      case pageIndex:
        ...
        this.ren.addClass(linkBtn, "text-[#f5f5f5]");
        break;
  ...
  public ngAfterViewInit() {
    console.log('starting but failing')
    this._rangeStart = 0;
    this._rangeEnd = this._showTotalPages - 1;
    this.matPag.initialized.subscribe((initialized) => {
      setTimeout(() => {
        this.initPageRange();
      }, 400);
    });
